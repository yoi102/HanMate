...
HanMate（汉语小伴）是一款面向一般学习者、包括儿童的中文学习应用。应用无需注册账号，没有广告、内购或面向开发者的使用行为分析服务。

### 本机处理的数据

你输入或导入的文字、拼音修正、收藏、学习设置以及录音保存在设备本地。应用使用这些数据提供阅读、注音、朗读、录音和备份功能，不将它们发送给开发者。手写笔迹在本机用于识别候选字。应用不会建立开发者可访问的云端个人资料库。

麦克风权限仅用于你主动开始的录音。拒绝权限后仍可使用不需要录音的功能。你可以在系统设置中撤销权限。录音不会自动上传。
...
只有在你主动导出、分享或保存备份时，选中的内容才会交给你指定的目标应用或文件位置。导出的文件可能含有个人文字和录音；接收方的处理方式由其政策决定。应用不会自动把备份发送给开发者。

你可以通过应用提供的管理功能删除个人内容或录音，通过系统设置清除应用数据，或者卸载应用。卸载与清理不会自动删除你已导出到其他位置的文件；这些副本需要自行管理。Android 已禁用应用的自动系统备份；Windows 或其他系统级备份设置由你和操作系统管理。

### 儿童与联系

应用不要求儿童提交姓名、邮箱、位置或其他身份资料，也不提供广告、公开用户动态或应用内聊天。家长或监护人应指导儿童使用录音、导入、分享及备份功能，避免在内容中包含不必要的个人信息。

如通过上述邮箱联系我们，我们会使用你主动提供的邮件内容来处理咨询。请不要发送密码、身份文件或不必要的儿童个人信息。你可以通过该邮箱提出隐私相关咨询、访问或删除请求；本机数据不由开发者托管，通常需要在你的设备上管理。

如应用的数据处理方式发生变化，我们会更新本政策及生效日期。

## English

HanMate is a Chinese learning app for general learners, including children. It requires no account and contains no advertising, in-app purchases or developer-operated usage analytics.

Text you enter or import, pinyin corrections, favorites, settings and recordings are stored on your device and used for reading, annotation, speech, recording and backups. They are not sent to the developer. Handwriting strokes are processed locally for character suggestions. The developer does not operate a cloud profile database for app users.

Microphone permission is used only when you start recording. Other features remain available without that permission, and you can revoke it in system settings. Recordings are not uploaded automatically.
...
If you email the developer, the information you voluntarily provide is used to respond to your inquiry. Do not send passwords, identity documents or unnecessary information about children. Contact **yoiri102@gmail.com** for privacy inquiries, access or deletion requests. App data is stored locally rather than hosted by the developer and generally needs to be managed on your device. We will update this policy and its effective date if data practices change.

## 日本語

HanMate は子どもを含む一般の学習者向けの中国語学習アプリです。アカウント登録は不要で、広告、アプリ内課金、開発者が運営する利用状況解析はありません。

入力・取り込みした文章、ピンインの修正、お気に入り、設定、録音は端末に保存し、読解、注音、読み上げ、録音、バックアップに使用します。開発者へ送信しません。手書きの筆跡は端末内で候補文字の認識に使います。開発者は利用者のクラウド個人データベースを運営していません。
...
子どもに氏名、メールアドレス、位置情報などの本人情報の入力を求めません。広告、公開タイムライン、アプリ内チャットはありません。録音、取り込み、共有、バックアップは保護者の指導のもとで使用し、不要な個人情報を文章や録音に含めないようにしてください。

開発者にメールでお問い合わせいただいた場合、任意で提供された情報を回答に使用します。パスワード、本人確認書類、不要な子どもの個人情報は送信しないでください。プライバシー、情報へのアクセスや削除に関するお問い合わせは **yoiri102@gmail.com** へご連絡ください。アプリ内のデータは開発者が保管していないため、原則としてご自身の端末で管理する必要があります。取り扱いが変わる場合は本ポリシーと適用日を更新します。
