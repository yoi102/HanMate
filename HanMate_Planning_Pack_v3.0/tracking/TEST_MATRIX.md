...
2026-09-23 W5 个人副本增量：隔离 Android 完整包从 retained 的工程语法条目正式保存个人副本，标题附 `_W5`；副本初始无源音轨，另行导入一秒合成 WAV 后为默认适用。卸载源包后语法列表 10→7 条，副本及音轨仍在；显式同版重装后列表回到 10 条，副本和音轨仍在。16 个自动拼音继续待审；正文/人工拼音修改、跨版本更新、真机及 Windows 原生 UI 未验。HM-T149 仅分范围 PASS，176 聚合与 45/48 不变。见[W5 证据](evidence/W5-REGRESSION-20260923.md)。

2026-09-23 W5 个人导入音轨增量：在隔离 Android 完整包中给已撤下资源条目导入一秒合成 WAV，保存为非默认的个人音轨。资源卸载后从 retained 正文进入，个人轨与标准轨并存；同版重装后仍为 5 条、1 hidden，两轨继续适用，标准轨保持默认。再单独恢复该条后详情为 5 条、0 hidden，两轨仍适用。该个人轨不是麦克风录音；真机路由、用户语音、草稿、个人正文副本和完整 P0 未验。HM-T152 仅分范围 PASS，176 聚合与 45/48 不变。见[W5 证据](evidence/W5-REGRESSION-20260923.md)。

2026-09-23 W5 工程音轨资源增量：隔离 Android 完整包将外部资源升至 1.0.2，合成一秒测试音在应用音轨页显示为默认且适用，点击后先显示播放中再显示播放完成；资源卸载后重启保持 0 条，显式同版重装恢复 5 条、1 hidden 与该音轨。Windows 独立临时库核对安装 WAV 的 SHA256 与包内一致，相关资源媒体/升级/管理 39/39 测试通过。音轨是合成提示音，不能代替教学听审、用户录音、真机音频路由或 Android 私有文件哈希。HM-T147/152 仅分范围 PASS；W5-01/05/07、176 聚合与 45/48 不变。见[W5 证据](evidence/W5-REGRESSION-20260923.md)。

2026-09-23 W5 资源升级局部回归：仓库原样例因与已导入工程内容共用图谱 ID 被正确拒绝；独立工程学习资源在隔离 Android 模拟器实装 1.0.0 共 4 条，撤下一条并重启后仍为 1 hidden；升级 1.0.1 后为 5 条、1 hidden，新增“中国”可见，原条目保持隐藏。同版重复安装显示已安装 5 条，旧版回退被拒且现有状态不变。Windows 全新临时库的正式安装器复验同一升级与撤下保持。HM-T141/142/146/147 均仅记分范围 PASS；无音轨、个人副本及完整平台 P0，真机本轮不可用。原 176 聚合与 45/48 不变。见[W5 证据](evidence/W5-REGRESSION-20260923.md)。

2026-09-23 W5 分享/迁移增量：隔离 Android 完整包从 24 条工程内容混选四类各一，导出内容、清单和文件哈希均核对；Windows 独立新库导入四条文档 JSON 完全一致，再导入复用四条。Android 默认 `.hanpack` 被系统追加 `.zip` 的问题已修复并在新完整包复验原生与兼容 ZIP 两条保存路径；单条语法原生包回导 0 新增、1 复用。分范围执行记录新增 HM-T165/166 行，正式音轨、Windows UI、真机和完整 P0 未验，原 176 聚合与 45/48 不变。见[W5 证据](evidence/W5-REGRESSION-20260923.md)。

2026-09-23 W5 多页焦点追加：Android 完整包 SHA256 `44F0638F000A87984F999A970F8828D548A0065C643321EA5F9CDA89B8AC698E` 在隔离模拟器 TalkBack+TAB 下，学习词语 7 卡、语法 3 卡、课文 4 卡、普通/字典收藏卡片均无逐行空白焦点；相应卡片点击及词语长按路径仍可达。语法/课文/字典详情的共用焦点修复在前一候选完整包已逐项验证。未完成手势、实际语音、所有页面或 Windows Narrator；原 176 聚合与 45/48 不变。见[W5 证据](evidence/W5-REGRESSION-20260923.md)。

2026-09-23 W5 追加：隔离 API 36 模拟器 TalkBack 启用后逐项 TAB 暴露字典释义的无名称行焦点，Android 行容器修复后新完整 APK 的词头、收藏、释义朗读及展开后路径前 18 个停靠点均为有名称按钮；未完成手势/真人听读/全页或真机验收。随附资源 141 条卸载后重启未自动重装、显式恢复后恢复 141 条并启用；1 条收藏引用卸载后可读、重装未重复，HM-T152 部分覆盖。Windows 服务层 10k 查询 30 次 P95 最高 57.43 ms；模拟器启动 30 次 ActivityManager P95 1836.4 ms；临时库恢复/清理故障 5/5 与 7/7 PASS。实体手机本轮不可用，参考机性能、音频路由与读屏 `NOT RUN`；原 176 聚合与 45/48 不变。见[W5 证据](evidence/W5-REGRESSION-20260923.md)。

2026-09-23 W5 前序完整包：隔离模拟器 200% 字号下确认字典标题及语言/页脚刷新；恢复字号 1.0 后“日”原文返回首 5 可见释义块位置一致。Windows 字典服务冷/热探针以及 Core 393、Infrastructure 378、Python 10+3 项 PASS；该包当时未执行 TalkBack/Narrator、资源/分享/迁移、完整 P0，后续 TalkBack 局部结果见上。原 176 聚合仍未执行。详见[W5 证据](evidence/W5-REGRESSION-20260923.md)。

2026-09-23当前增量：Android隔离4 GB模拟器长“日”释义新增3+5轮操作PASS，Windows/Android原文返回前后首5个可见释义块位置一致；HM-D019按用户指令`CLOSED_BY_REQUEST`，并非已定位根因或全平台无崩溃。见[字典返回](evidence/DICTIONARY-RETURN-20260923.md)及[结案](evidence/HM-D019-CLOSURE-20260923.md)。原176聚合NOT RUN，其他W5平台矩阵仍待执行。
