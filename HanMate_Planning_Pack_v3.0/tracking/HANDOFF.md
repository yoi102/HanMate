...
2026-09-23 W5 个人副本接续：当前隔离 `emulator-5554` 上工程资源 1.0.2 已重装（上次恢复的条目为 0 hidden）；另有独立个人语法副本 `“在”表示位置_W5`，其完整单元绑定单独导入的合成 WAV。源资源卸载后副本与音轨保留，同版重装后仍在；原始 UI 见[W5 证据](evidence/W5-REGRESSION-20260923.md)。副本自动拼音 16 字未终审；更高版本更新、人工校正、Windows 原生 UI、真机与完整 P0 待验，W5-01/05/07 及 45/48 不变。本轮 ADB 曾同时出现 vivo V2408A 和其他模拟器，但未操作它们；后续 raw ADB 必须显式指定 `-s emulator-5554`。

2026-09-23 W5 个人音轨接续：隔离模拟器中，“在”条目的个人导入合成 WAV 与 1.0.2 标准工程音轨在资源卸载后的 retained 正文、以及显式同版重装后均并存；个人轨适用但未设默认，标准轨继续默认。重装详情为 5 条/1 hidden/优先级 100；随后只恢复该条，当前模拟器详情为 5 条/0 hidden，两轨仍适用。见[W5 记录](evidence/W5-REGRESSION-20260923.md)。这不是用户麦克风录音或听审，仍缺草稿、个人正文副本、Windows 原生 UI、真机和完整 P0；W5-01/05/07 保持进行中，45/48。

2026-09-23 W5 含音频资源接续：工程资源 1.0.2 新增明确标识的 1 秒合成测试音。当前隔离 Android 模拟器已完成 1.0.1→1.0.2、音轨默认适用与播放完成、卸载后重启不自动重装、显式同版重装后 5 条/1 hidden 与音轨恢复；Windows 全新临时库精确核对包内/安装 WAV 哈希。当前模拟器保留 1.0.2 工程包，未清用户资料；APK 仍为 `09ED5EEEAAC557ECA52DB38C26D3065489AC28D80649FBFFECDCFB68A606BE46`。证据见[W5 记录](evidence/W5-REGRESSION-20260923.md)。尚缺真实用户音轨/草稿/个人副本、Windows 原生 UI、真机音频路由及完整 P0；W5-01/05/07 均未结项，45/48。

2026-09-23 W5 资源升级：`tools/make_w5_resource_upgrade_fixture.py` 的独立 UUID 双版本工程包已在 Windows 临时服务宿主及当前完整 APK 的隔离模拟器上验证；1.0.0 的 4 条中撤下一条，重启后保持，1.0.1 新增“中国”到 5 条，仍有 1 hidden。仓库原样例与个人导入工程内容碰 ID 的 `RESOURCE_CONTENT_CONFLICT` 是正确保护。原始截图/XML、包哈希、Windows 探针和 66 项相关测试见[W5 证据](evidence/W5-REGRESSION-20260923.md)。本轮没有实体手机；个人副本/音轨/草稿、Windows 原生 UI 和完整适用 P0 尚待验，W5-01/05/07 保持进行中、45/48。

2026-09-23 W5 分享/迁移接续：当前完整 Android x64 Release APK SHA256 `09ED5EEEAAC557ECA52DB38C26D3065489AC28D80649FBFFECDCFB68A606BE46`，已覆盖安装到隔离模拟器且拉取哈希相同。四类工程包从 Android 实际导出并在全新 Windows 服务宿主精确导入/复用；默认 `.hanpack` 与兼容 `.hanpack.zip` 保存及单条回导通过，见[W5 证据](evidence/W5-REGRESSION-20260923.md)。样例不含音轨，不能替代 Windows 原生 UI、真机/音频、资源升级与完整 P0；W5-01/05/07 均继续进行中，45/48。

2026-09-23 Windows 原生窗口补充：工作区 Release 拼音首页 UIA 与截图可见；引导按钮的键盘 Enter / UIA Invoke 后未观察到引导遮罩，一次实际鼠标点击被窗口归属保护拒绝，故 HM-T007 仍 `NOT RUN`，不单凭现象定性为应用缺陷。进程已正常退出、两个原数据库哈希与启动前备份一致，见[W5 证据](evidence/W5-REGRESSION-20260923.md)。本轮没有 Android 真机，W5-01/05/07 仍进行中，45/48 不变。

2026-09-23 W5 多页焦点追加：字典之外的语法/课文详情与学习/收藏列表也发现无名称 Android RecyclerView 行焦点，已复用原生行焦点处理。当前完整 APK SHA256 `44F0638F000A87984F999A970F8828D548A0065C643321EA5F9CDA89B8AC698E`；词语/语法/课文及普通/字典收藏卡片在隔离模拟器 TalkBack+TAB 局部复验有名称，点击/长按仍可导航。原生选中行的资源安装预览与搜索结果未改；TalkBack 手势/真人听读、Windows Narrator 和真机仍待验。见[W5 证据](evidence/W5-REGRESSION-20260923.md)；45/48 保持。

2026-09-23 W5 追加：新完整 Android APK 修复字典释义无名称原生行焦点；隔离模拟器 TalkBack+TAB 的词头、收藏、释义及展开后路径通过局部复验。随附学习资源 141 条卸载、重启保持未安装、显式恢复后恢复 141 条并启用；追加 1 条收藏引用时卸载保留该内容，恢复后收藏未重复，测试收藏已移除。用户录音、草稿、撤下条目及跨版本升级仍待验。Windows 服务层 10k 查询、模拟器 30 次启动与临时库恢复/清理故障已有分范围证据，见[W5 记录](evidence/W5-REGRESSION-20260923.md)。用户确认本轮没有 Android 真机；模拟器结果不代表实体参考机性能、音频路由或完整读屏。W5-01/05/07 保持进行中，45/48。

2026-09-23 W5 接续：最终完整开发签名包的 Android 200% 字典标题、连续语言切换、日英页脚，以及恢复 100% 后原文返回保位已聚焦 PASS；模拟器已恢复英语和系统字号 1.0。自动回归 Core 393/393、Infrastructure 378/378，Python 审核 10/10、拼音 3/3，两平台 Release 构建 PASS。原始临时证据和仍待的平台范围见[W5 聚焦记录](evidence/W5-REGRESSION-20260923.md)。下一步在隔离设备上推进资源管理/分享/迁移，补 Android TalkBack 与 Windows Narrator、性能和 P0；不要将服务探针或 UI 局部通过计为 W5-01/05/07 完成。

当前HM-D019已按用户2026-09-23指令以`CLOSED_BY_REQUEST`结案，**不是根因修复**；追加5轮隔离Android压力PASS，具体残余风险见[结案记录](evidence/HM-D019-CLOSURE-20260923.md)。后续若出现同类原生崩溃，保留tombstone/包哈希并重新开缺陷。当前继续推进W5-01/05/07；不要把HM-D019结案转换为完整P0、真机/读屏或商店门禁通过。

2026-09-23结案前接续记录：`DictionaryEntryPage`同语言从原文/设置返回时沿用原列表，语言确有变化才重绘；隔离Android 4 GB模拟器对“日”长释义3轮压力，以及Android/Windows原文返回保位PASS，Windows/Android Release构建PASS。首次2 GB模拟器`uiautomator`超时伴随内存耗尽，不能归因应用；当时HM-D019仍OPEN，现已按用户要求结案，真机/读屏/完整176矩阵仍待验。当前审核清单重新生成并校验：1,394对象均待审，词典293批0条终审；规划包构建及16组校验通过。原始UI证据在TEMP、结论见[本轮记录](evidence/DICTIONARY-RETURN-20260923.md)，UI专用裁剪APK不能验声音或用于商店。45/48保持；旧D8/D9交付身份不被本轮替换。

拼音向导修订（2026-09-20，替代下面的两步版本）：已改为真实目标镂空高亮、箭头与提示的跨页四步引导，单击拼音→长按拼音→单击例字→长按例字打开字典。按用户要求遮罩60%透明，提示区独立底色保持可读。Windows Release/Android Debug通过；模拟器四手势及实际透明度验证通过，实体手机未连接。最终完整包和专用模拟器包身份、范围见[本轮证据](evidence/PINYIN-SPOTLIGHT-20260920.md)。不改变45/48。

拼音使用向导（2026-09-20）：新增可实际操作的单击/长按两步卡片，首次显示、可跳过、完成状态本机保存，分组右侧问号重看。Windows Release/Android Debug通过；模拟器用排除大型语音模型的专用验证包完成布局、播放调用、长按导航、关闭/重看/冷启动状态检查，完整包未能覆盖安装模拟器（空间不足），真机未连接。完整APK身份、当前模拟器包范围及旧快速部署目录备份见[证据](evidence/PINYIN-GUIDE-20260920.md)。不恢复旧覆盖程序集；45/48保持。

最新通用连笔补充（2026-09-20）：生/金的多处连笔不再受“最多少两次抬笔”限制，现最多六次、每条对应连续1—3笔、模板最多12画；按比例惩罚并提前终止无望路径。HMI4数据/哈希不变。数据来源按钮及专用弹页移除，撤销/清空保留。20项手写测试、独立生/金及49个连接模板样本已验；最终设备/构建见[记录](evidence/HANDWRITING-GENERAL-20260920.md)。不是全字库真实准确率，45/48不变。

最新手写优化（2026-09-20）：模板改用HMI4预处理资产，原medians/许可保持，重建用 `tools/HanMate.HandwritingCompiler`。搜索提前预热，行缓存随epoch失效；笔顺/方向容错、转角断笔、子的连笔和越界松手保留已实施。37项受影响自动测试通过；最终包/设备证据见[记录](evidence/HANDWRITING-TOLERANCE-20260920.md)。不把85个模板扰动样本当作真实手写准确率，45/48与其他未验门禁保持。

最新真机语音修复（2026-09-20）：用户重新授权实体手机调试，v4 Debug已安装vivo V2408A；Melo、系统TTS及Kokoro FP32男女声均获用户清楚试听确认，字典点读/停止/切换有日志。67项测试与两目标构建通过。APK身份、模型迁移、性能与未验边界见[证据](evidence/DEVICE-SPEECH-20260920.md)。保留用户当前声音选择；旧快速部署覆盖目录已移动备份，不恢复，否则会运行旧程序集。以下D8交付档案不包含本轮修复，旧“新APK未安装/调试暂停”不覆盖本轮。仍45/48，R14缺Melo中文男声。

Android启动修复（2026-09-20）：Application新增`Maui.MainTheme.NoActionBar`；Debug构建及用户授权的模拟器冷启动/首页五标签检查通过，TextAppearance异常未复现。当前模拟器为本轮Debug包，实体手机未覆盖安装，其他设备调试仍按原约束暂停。见[evidence/ANDROID-STARTUP-THEME-20260920.md](evidence/ANDROID-STARTUP-THEME-20260920.md)。
...
更新：2026-09-23。**45 DONE / 3 IN_PROGRESS / 0 TODO**，W5-05已恢复执行聚焦回归，完整P0仍待验。ADR-106内部开发交付口径，正式审校/签名/商店发布后置；Windows/Android范围，iOS按ADR-105延期。见[W5证据](evidence/W5-REGRESSION-20260923.md)、[D8证据](evidence/STAGE-D8-20260920.md)与[交付说明](../delivery/README.md)。

## 当前交付

仓库 `artifacts/internal-20260920-d8-v2/delivery-final/` 为最终归档：Windows x64发布ZIP、Android开发签名APK、源码ZIP、构建日志、逐文件清单、离线审核材料与恢复/隐私指南。`delivery-manifest.json`记录SHA256；两个ZIP内每个条目均完整核验。构建前后实际编译输入一致，归档管理文档按最终45项状态更新。无Git元数据，不声称提交或正式签名。

例字对应词已补42个；251/256处同字同读音配对，显示时例词紧随例字。417例字词/394录音，新增42词缺整词录音；不以单字录音冒充整词。1,444审核对象均待审。

## 后续工作

1. 测试恢复后推进W5-01/05/07：两平台设备性能/读屏、完整P0与HM-D019根因、资源管理/分享/迁移回归。当前176聚合NOT RUN与82执行行均未改。
2. 按 `deferred-release-gates.json` 的RG-01—07逐项完成正式内容/权利、发行身份与签名安装升级、商店声明、设备质量/回归与最终发布归档；内部45不使发布预检自动放行。
3. 新APK未安装，新Windows未启动；本轮没有测试录音或改变设备状态。D7的实机Debug包/Windows发布采集及已恢复权限以其原始证据为准，不把旧PID当作当前运行进程。保留真实资料、声音和模型；签名不兼容时不得卸载绕过。

复现用 `tools/build_internal_delivery.ps1`（全新输出目录），顺序构建；交付校验用 `tools/package_internal_delivery.py verify --output <交付目录>`。D8未执行应用测试；D9自动回归按上述范围执行，不写成UI PASS。
